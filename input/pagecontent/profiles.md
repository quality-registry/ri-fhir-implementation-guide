...
| [Self-Reported Value Aggregation](StructureDefinition-self-reported-value-aggregation-profile.html) | Figures the registry calculates from self-reported readings, with the readings used and the registry's assessment of them in `interpretation`. Currently describes blood-pressure averages over a 7, 14 or 30-day window, with time in range. |
