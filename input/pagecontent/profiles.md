...
| [Self-Reported Value Aggregation](StructureDefinition-self-reported-value-aggregation-profile.html) | Figures the registry calculates from self-reported readings, linked to the readings they were derived from, with the registry's assessment in interpretation. |
...
| Profile | Why it exists |
| --- | --- |
| [Patient Reported Outcome Questionnaires](StructureDefinition-patient-reported-outcome-questionnaires.html) | Fixes the shape shared by the four registry instruments: a versioned definition addressed by its canonical URL, a coded instrument identifier, and a flat list of coded, required, non-repeating questions. |
